...
**現況**:`modules/triage/index.js` 已刪除內嵌分類器(`buildJudgePrompt`/`loadProjectContext`/`callAiJudge`/`extractJudgeJson`/`judgeMessage` 及域門檻 `enabledFor`/`aiConfigured`),改於分類段呼叫 **`platform.classify`**(construction 於 `init` 掛上;非工程租戶回 `null`),triage 只保留過濾層1、過濾層2(高信心閒聊自動歸檔)+ 寫回訊息頁(`writeJudgement`)。
**驗證**:`node --check modules/triage/index.js` 通過;`node tools/dryrun-triage.mjs`:**9/9**(mock 網路)——層1 短路、委派 classify、層2 自動歸檔、未分類交棒 queue、無分類器租戶只跑層1、例外吞掉、守門、construction 掛鉤容錯。
