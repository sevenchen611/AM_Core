...
async function init(){try{$('retry').classList.add('hidden');if(!window.liff)throw Error('LINE 登入元件載入失敗，請關閉後從群組連結重新開啟。');await within(liff.init({liffId:DATA.liffId}),10000,'LINE 身分驗證逾時，請按重新驗證或從群組重新開啟。');if(!liff.isLoggedIn()){liff.login({redirectUri:location.href});return}token=liff.getAccessToken?.()||'';await within(api({action:'identify',sessionToken:DATA.sessionToken}),12000,'LINE 身分驗證逾時，請按重新驗證或從群組重新開啟。');identified=true;$('identity').className='status';$('identity').textContent='LINE 身分已驗證';$('form').classList.remove('hidden')}catch(error){$('identity').className='status error';$('identity').textContent=error.message||'LINE 身分驗證失敗';$('retry').classList.remove('hidden')}}
