...
function applyTemplate(value){if(!value||!Array.isArray(value.lines)||!value.lines.length)return false;$('lines').replaceChildren();value.lines.forEach(line=>addLine(line.description,String(line.amount??'')));$('note').value=value.note||'';if(isInsurance()){$('companyExpense').value=String(value.companyExpenseAmount??'');$('employeeRecoverable').value=String(value.employeeRecoverableAmount??'');}$('period').value='';$('dueDate').value='';$('attachments').value='';syncTotal();$('templateStatus').className='status';$('templateStatus').textContent='已套用你的個人範本；請補上本次請款月份、日期與附件。';return true}
...
let submittedClaimId='',uploadedAttachmentCount=0;
