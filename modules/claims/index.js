...
async function rentalPersonalClaimTemplate(tenant, session, action, template = undefined) {
...
async function handleLiff(req, res, { pathname, url, tenant = null, tenants = [] }) {
  ...
  try {
    ...
    if (action === 'getTemplate') {
      const result = await rentalPersonalClaimTemplate(sessionTenant, session, 'get');
      return sendJson(res, 200, { ok: true, template: result.template || null, updatedAt: result.updatedAt || '' });
    }
    if (action === 'saveTemplate') {
      const result = await rentalPersonalClaimTemplate(sessionTenant, session, 'upsert', body.template);
      return sendJson(res, 200, { ok: true, saved: result.saved === true, updatedAt: result.updatedAt || '' });
    }
