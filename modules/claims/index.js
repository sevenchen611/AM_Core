...
const api=async(body)=>{const r=await fetch(location.pathname,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({...body,liffAccessToken:token})});const j=await r.json().catch(()=>({}));if(!r.ok)throw Error(j.error||'操作失敗');return j};
