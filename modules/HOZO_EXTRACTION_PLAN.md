...
```
底座先行：core/llm.js ✅(2ad91e3)  →  core/event-queue.js
   （llm 抽象一做完，meetings 的手工備援也能收斂進來）
   ＋ tenants config 區塊 ✅(8144216)：行業味的落腳處

資料鏈：H1 conversations → H3 control-tasks → H2 extraction
   （extraction「寫任務」，所以任務模型要先立起來；
     初版計畫把 H2 排在 H3 前面是錯的。）
...
**可直接派工的交接單**：`modules/HOZO_DISPATCH.md`
