...
### 6.1 ⚠️ 陷阱：LINE 的 groupId 是「每個 channel 各自發放」

平台路由**完全靠 groupId 比對**（`core/router.js:23` → `LINE 群組 ID` equals）。
換 OA ＝ 換 channel ⇒ **同一個 LINE 群，在葉小蝸眼中的 groupId 與 HOZO Jr. 眼中的不同**。
HOZO「LINE 群組選項」庫裡現存的 6 組 `C…` 值**全部作廢**，必須重抓。

> **零成本驗證**：把葉小蝸拉進任一個 HOZO 群、發一句話，看平台 log 的 groupId 是否等於庫裡舊值。
> 相同 → 省一大段工；不同 → 照下面走。（先當作「一定要重抓」來規劃。）
...
4. 葉小蝸逐一拉進 6 個 HOZO 群 → 從平台 log 抓新 groupId → 寫進綁定庫(狀態=啟用)
5. 綁定驗證通過後，才停 HOZO Jr. 的 webhook（回退＝把 webhook 指回 HOZO_AM）
```

**HOZO 現有 6 個群**：HOZO 公司群／HOZO好住工務群組／HOZO品牌設計／HOZO好住寓好公司設立／HOZO好住公司設立協助／明義街46號2之2
