...
async function openReview(){state.token=new URLSearchParams(location.hash.slice(1)).get('token')||'';history.replaceState(null,'',location.pathname);if(!state.token)throw new Error('草約審閱連結不完整');const review=await call('/contract-review/api/open',{token:state.token});render(review);show('合約與附件已準備完成，請點選檔案開啟查看。');}
