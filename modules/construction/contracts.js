...
<style>
  ...
  * { box-sizing:border-box; margin:0; }
  ...
  .attachment-chip{display:inline-flex;align-items:center;position:relative;background:#eef2f0;border-radius:6px}.attachment-chip .rowbtn{margin-left:0}.attachment-remove{border:0;background:#b8322a;color:#fff;border-radius:50%;width:20px;height:20px;line-height:18px;padding:0;margin:0 5px 0 1px;cursor:pointer;opacity:0;transform:scale(.85);transition:opacity .15s,transform .15s}.attachment-chip:hover .attachment-remove,.attachment-chip:focus-within .attachment-remove{opacity:1;transform:scale(1)}@media(pointer:coarse){.attachment-remove{opacity:1;transform:scale(1)}}
</style>
</head>
<body>
...
let DATA = null;
let CURRENT = null;
let WORKFLOW = { row:null, contract:null, detail:null, reviews:[], archives:[], archivesAvailable:false, files:{}, creatingVersion:false };
let TEMPLATES = [];
...
function closeWorkflow(){ document.getElementById('workflow-modal').hidden=true; WORKFLOW={row:null,contract:null,detail:null,reviews:[],archives:[],archivesAvailable:false,files:{},creatingVersion:false}; }
...
async function openWorkflow(id,createNext=false){
  ...
  WORKFLOW={row:found.row,project:found.project,contract:null,detail:null,reviews:[],archives:[],archivesAvailable:false,files:{},creatingVersion:false,revisionReview:null};
...
function versionComposerHtml(nextVersion){
  ...
  return '<div class="workflow-state">建立 V'+nextVersion+'：系統會從 V1 起累積保留所有歷史附件；這次上傳的文件會成為目前使用版本，舊檔仍列在歷程附件中，不會消失或被覆寫。資料未完整時不得送審或簽發。</div>'+sourceHtml+'<div class="workflow-grid">'
