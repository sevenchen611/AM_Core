...
async function workflowAssignPartyASigner(){try{const signer=document.getElementById('wf-assign-party-a')?.value;if(!signer)throw new Error('請選擇個人甲方的 LINE 帳號');await apiV2('signing-sessions/'+encodeURIComponent(WORKFLOW.row.signingSessionId)+'/assign-party-a',{method:'POST',body:{partyASignerLineUserId:signer}});alert('已綁定個人甲方 LINE 簽署人。甲方現在可從原工程 LINE 群組連結進入簽名。');}catch(error){alert(error.message);}}
async function workflowComplete(){const individual=currentPartyAProfileType()==='individual';const button=document.getElementById('party-a-complete');try{if(button){button.disabled=true;button.textContent=individual?'驗證甲乙雙方線上簽名並歸檔中…':'確認並歸檔中…';}await apiV2('signing-sessions/'+encodeURIComponent(WORKFLOW.row.signingSessionId)+'/confirm-complete',{method:'POST',body:{}});WORKFLOW.row.signingStatus='completed';alert(individual?'甲方與乙方的線上簽名均已驗證；最終合約 PDF 與證據收據已封存。':'已完成我方確認；最終合約 PDF 與證據收據已封存。');renderWorkflow();}catch(error){if(button){button.disabled=false;button.textContent=individual?'驗證甲乙雙方線上簽名並產生最終歸檔':'確認簽署並產生最終歸檔';}alert(error.message);}}
load();
</script>
</body>
</html>`;
