...
function partyAAssetBadges(profile){const assets=profile.assets||{};const labels={large_seal:'公司大章',signature:'個人簽名'};return Object.keys(labels).filter(key=>assets[key]).map(key=>'<span class="asset-badge">✓ '+labels[key]+'</span>').join('');}
function renderPartyAProfiles(){
  ...
  main.innerHTML='<div class="panel"><h3>甲方主檔</h3><div class="hint">公司甲方保存公司名稱、統編、負責人、地址及公司大章；個人甲方保存姓名、地址及簽名。選入合約時會複製成該版本快照，日後修改主檔不會改寫已建立或已簽署的合約。</div><div class="workflow-actions"><button class="btn" onclick="showPartyAProfileForm()">＋ 新增甲方</button></div>'+form+'<div class="profile-grid">'+cards+'</div>'+(cards?'':'<div class="empty">尚未建立甲方主檔。</div>')+'</div>';
}
function partyAProfileFormHtml(id){const profile=PARTY_A_PROFILES.find(item=>item.id===id)||{profileType:'company',assets:{}};PARTY_A_UPLOADS=JSON.parse(JSON.stringify(profile.assets||{}));const type=profile.profileType||'company';const assetState=(kind,label)=>'<label class="'+(kind==='signature'?'individual-only':'company-only')+'">'+label+'<input type="file" accept="image/png,image/jpeg" onchange="uploadPartyAAsset(&quot;'+kind+'&quot;,this)"><div id="pa-state-'+kind+'" class="file-state">'+(PARTY_A_UPLOADS[kind]?'✓ 已保存 '+esc(PARTY_A_UPLOADS[kind].name||label):'尚未上傳')+'</div></label>';return '<div id="party-a-form" class="workflow-box profile-form" data-type="'+type+'"><h4>'+(id?'編輯甲方主檔':'新增甲方主檔')+'</h4><div class="workflow-grid"><label>甲方類型<select id="pa-type" onchange="partyATypeChanged()"><option value="company" '+(type==='company'?'selected':'')+'>公司（大章）</option><option value="individual" '+(type==='individual'?'selected':'')+'>個人（簽名）</option></select></label><label>顯示名稱<input id="pa-display" value="'+esc(profile.displayName||'')+'" placeholder="選單中看到的名稱"></label><label>法定名稱／姓名<input id="pa-legal" value="'+esc(profile.legalName||'')+'"></label><label class="company-only">統一編號<input id="pa-tax" value="'+esc(profile.taxId||'')+'" inputmode="numeric" maxlength="8"></label><label class="company-only">負責人<input id="pa-responsible" value="'+esc(profile.responsiblePerson||'')+'"></label><label>代表人／簽約人<input id="pa-representative" value="'+esc(profile.representative||'')+'"></label><label class="individual-only">身分證字號（需要時）<input id="pa-identity" value="'+esc(profile.identityNumber||'')+'"></label><label class="wide">地址<input id="pa-address" value="'+esc(profile.address||'')+'"></label>'+assetState('large_seal','公司大章（必填）')+assetState('signature','個人簽名（必填）')+'</div><div class="workflow-actions"><button id="pa-save" class="btn" onclick="savePartyAProfile(&quot;'+esc(id)+'&quot;)">儲存甲方</button><button class="btn ghost" onclick="cancelPartyAProfileForm()">取消</button></div></div>';}
