...
function versionComposerHtml(nextVersion){
  ...
  return '<div class="workflow-state">建立 V'+nextVersion+'：目前有效文件與付款／驗收資料已從 V'+(latest?.versionNo||'1')+' 帶入；沒有調整的項目不必重新上傳或重填。替換與停用只影響 V'+nextVersion+' 起的有效內容，舊版與雜湊證據仍完整保留。</div>'+sourceHtml+'<div class="workflow-grid">'
    +(nextVersion===1?templatePickerBox():'')+existingDocumentBox('contract_body','合約本文',pkg.contractBody?[pkg.contractBody]:[],nextVersion,'replace')+existingDocumentBox('construction_drawing','施工圖',pkg.constructionDrawings||[],nextVersion,'add')+existingDocumentBox('quotation','報價單',pkg.quotation?[pkg.quotation]:[],nextVersion,'replace')
