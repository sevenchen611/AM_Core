    ...
    try {
      ...
      for (const project of projects) {
        for (const row of project.rows) {
          ...
          row.partyASignerLineUserId = workflow?.party_a_signer_line_user_id || '';
          row.partyASigned = workflow?.party_a_signed === true;
...
<style>
  ...
  * { box-sizing:border-box; margin:0; }
  ...
  .project-drawing-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(235px,1fr));gap:10px;margin-top:10px}.project-drawing-card{display:grid;grid-template-columns:72px 1fr;gap:10px;align-items:start;border:1px solid #cfe0d6;border-radius:10px;padding:9px;background:#fff}.project-drawing-card.selected{border-color:var(--green);background:#f0faf4}.project-drawing-card.unavailable{opacity:.65}.project-drawing-thumb{width:72px;height:72px;border-radius:7px;border:1px solid var(--line);object-fit:cover;background:#f4f6f5}.project-drawing-fileicon{width:72px;height:72px;display:grid;place-items:center;border-radius:7px;border:1px solid var(--line);background:#f4f6f5;font-size:26px}.project-drawing-card label{display:flex;gap:6px;color:var(--ink);font-weight:600}.project-drawing-card input{width:auto;margin:2px 0 0}.source-badge{display:inline-block;padding:2px 6px;border-radius:999px;background:#e8f4ec;color:#246641;font-size:10px}
  .party-a-signing-box{width:100%;margin-top:12px;border:1px solid #9dcdb0;border-radius:12px;padding:13px;background:#f3faf6}.party-a-signing-box h4{margin:0 0 6px}.party-a-signature-wrap{height:clamp(280px,42vh,420px);margin:10px 0;border:2px solid var(--green);border-radius:10px;background:#fff;overflow:hidden;touch-action:none}.party-a-signature-wrap canvas{display:block;width:100%;height:100%;touch-action:none;cursor:crosshair}.party-a-consent{display:flex;gap:8px;align-items:flex-start;font-size:12px;line-height:1.55}.party-a-consent input{width:auto;margin-top:3px}@media(max-width:720px){.party-a-signature-wrap{height:340px}}
</style>
</head>
<body>
...
let PARTY_A_UPLOADS = {};
let PROJECT_DRAWING_PICKER = { contractId:'', items:[], selected:[], loading:false, open:false, error:'' };
...
function closeWorkflow(){ document.getElementById('workflow-modal').hidden=true; WORKFLOW={row:null,contract:null,detail:null,reviews:[],archives:[],archivesAvailable:false,files:{},pendingExclusions:[],creatingVersion:false}; PROJECT_DRAWING_PICKER={contractId:'',items:[],selected:[],loading:false,open:false,error:''}; }
...
async function openWorkflow(id,createNext=false){
  ...
  PROJECT_DRAWING_PICKER={contractId:'',items:[],selected:[],loading:false,open:false,error:''};
  ...
  try{
    ...
    try{applyWorkflowSigningControl(await apiV2('contracts/'+encodeURIComponent(contractId)+'/control'));}catch{}
    ...
    LINE_ATTACHMENT_REVIEW={versionId:'',items:[],loading:false,importing:false};
    renderWorkflow();
    if(WORKFLOW.detail.latestVersion)loadLineAttachmentCandidates();
...
function versionComposerHtml(nextVersion){
  ...
  return '<div class="workflow-state">建立 V'+nextVersion+'：目前有效文件與付款／驗收資料已從 V'+(latest?.versionNo||'1')+' 帶入，合約基本資料也會一併沿用；沒有調整的項目不必重新上傳或重填。替換與停用只影響 V'+nextVersion+' 起的有效內容，舊版與雜湊證據仍完整保留。</div>'+sourceHtml+contractFieldsEditorWithPartyAHtml(pkg)+'<div class="workflow-grid">'
    +(nextVersion===1?templatePickerBox():'')+existingDocumentBox('contract_body','合約本文',bodyFiles,nextVersion,'replace')+existingDocumentBox('construction_drawing','施工圖',drawingFiles,nextVersion,'add')+projectDrawingPickerHtml()+existingDocumentBox('quotation','報價單',quotationFiles,nextVersion,'replace')
...
function finalArtifactHtml(version){if(WORKFLOW.row.signingStatus!=='completed')return '';return '<div class="workflow-box final-artifact"><h4>最終簽署文件與證件附件</h4><div class="hint">甲乙雙方簽署與最終歸檔均已完成；系統會在開啟時重新驗證私有權限、合約案場範圍與 SHA-256。身分證只限有合約查看權限的內部人員開啟。</div><div class="workflow-actions"><a class="btn" target="_blank" rel="noopener" style="text-decoration:none" href="'+esc(internalDocumentHref(version,'final-signed-pdf'))+'">開啟最終簽署合約 PDF</a><a class="rowbtn" target="_blank" rel="noopener" style="text-decoration:none;display:inline-block" href="'+esc(internalDocumentHref(version,'identity-document-front'))+'">開啟簽約人身分證正面原圖</a><a class="rowbtn" target="_blank" rel="noopener" style="text-decoration:none;display:inline-block" href="'+esc(internalDocumentHref(version,'identity-document-back'))+'">開啟簽約人身分證反面原圖</a><a class="rowbtn" target="_blank" rel="noopener" style="text-decoration:none;display:inline-block" href="'+esc(internalDocumentHref(version,'evidence-receipt'))+'">開啟簽署證據收據</a></div></div>';}
function contractDocumentsHtml(version){const internal=internalPreviewHtml(version)+'<div id="line-attachment-review">'+lineAttachmentReviewHtml(version)+'</div>';if(WORKFLOW.row.signingStatus!=='completed')return internal;return finalArtifactHtml(version)+internal.replace('內部審查文件（唯讀）','簽署前凍結版本與附件').replace('開啟完整合併合約 PDF','開啟簽署前完整合併 PDF');}
let LINE_ATTACHMENT_REVIEW={versionId:'',items:[],loading:false,importing:false};
function lineAttachmentReviewHtml(version){const current=LINE_ATTACHMENT_REVIEW.versionId===version.id;const items=current?LINE_ATTACHMENT_REVIEW.items:[];const labels={quotation:'報價單',construction_drawing:'施工圖',other:'其他附件'};return '<div class="workflow-box" style="margin-top:14px"><h4>LINE 群組檔案待審查</h4><div class="hint">群組原始檔先供內部查看；勾選保留後會建立下一版合約附件。未勾選的檔案不提供廠商，移除附件也不刪除 LINE／Drive 原始檔。DOCX、XLSX、DWG 保留原始檔下載，不轉成 PDF 頁面。</div><button type="button" class="rowbtn" onclick="loadLineAttachmentCandidates()" '+(LINE_ATTACHMENT_REVIEW.loading?'disabled':'')+'>'+(LINE_ATTACHMENT_REVIEW.loading?'正在載入…':'載入／重新整理群組檔案')+'</button><div class="carried-files">'+items.map(item=>'<div class="carried-file"><label><input type="checkbox" data-line-attachment="'+esc(item.id)+'" '+(!CAN_MANAGE||!item.available||item.included?'disabled':'')+'> '+(item.available?'<a target="_blank" rel="noopener" href="'+esc(internalDocumentHref(version,'line-attachments/'+encodeURIComponent(item.id)))+'">'+esc(item.name)+'</a>':esc(item.name))+'</label><span class="hint">'+esc(item.sourceEvidence.sender||'未知上傳人')+'｜'+esc(item.sourceEvidence.time||'')+'｜'+esc(item.included?'已納入合約':item.reason||'待選取')+'</span><select data-line-category="'+esc(item.id)+'" '+(!CAN_MANAGE||!item.available||item.included?'disabled':'')+'>'+Object.keys(labels).map(key=>'<option value="'+key+'" '+(item.category===key?'selected':'')+'>'+labels[key]+'</option>').join('')+'</select></div>').join('')+'</div>'+(current&&!items.length?'<div class="file-state">目前没有已保存的群組檔案。</div>':'')+(CAN_MANAGE?'<button type="button" class="btn" onclick="importLineAttachmentCandidates()" '+(LINE_ATTACHMENT_REVIEW.importing?'disabled':'')+'>將勾選檔案保留為下一版附件</button>':'')+'</div>';}
function renderLineAttachmentReview(error){const panel=document.getElementById('line-attachment-review');const latest=WORKFLOW.detail?.latestVersion;if(panel&&latest)panel.innerHTML=lineAttachmentReviewHtml(latest)+(error?'<div class="hint">群組檔案載入失敗：'+esc(error)+'</div>':'');}
async function loadLineAttachmentCandidates(){const latest=WORKFLOW.detail?.latestVersion;if(!latest||LINE_ATTACHMENT_REVIEW.loading)return;const contractId=WORKFLOW.contract.id;LINE_ATTACHMENT_REVIEW={versionId:latest.id,items:[],loading:true,importing:false};renderLineAttachmentReview();try{const items=await apiV2('contracts/'+encodeURIComponent(contractId)+'/versions/'+encodeURIComponent(latest.id)+'/line-attachments');if(WORKFLOW.contract?.id!==contractId||WORKFLOW.detail?.latestVersion?.id!==latest.id)return;LINE_ATTACHMENT_REVIEW={versionId:latest.id,items,loading:false,importing:false};renderLineAttachmentReview();}catch(error){if(WORKFLOW.contract?.id!==contractId||WORKFLOW.detail?.latestVersion?.id!==latest.id)return;LINE_ATTACHMENT_REVIEW.loading=false;renderLineAttachmentReview(error.message);}}
async function importLineAttachmentCandidates(){
  const latest=WORKFLOW.detail?.latestVersion;
  if(!latest||LINE_ATTACHMENT_REVIEW.importing||LINE_ATTACHMENT_REVIEW.versionId!==latest.id)return;
  if(WORKFLOW.creatingVersion){alert('請先儲存或取消正在編輯的新版，再保留群組附件。');return;}
  const contractId=WORKFLOW.contract.id;
  const selections=Array.from(document.querySelectorAll('[data-line-attachment]:checked')).map(el=>({id:el.dataset.lineAttachment,category:Array.from(document.querySelectorAll('[data-line-category]')).find(select=>select.dataset.lineCategory===el.dataset.lineAttachment)?.value||'other'}));
  if(!selections.length){alert('請先勾選要保留的群組檔案。');return;}
  if(selections.length>30){alert('每次最多保留 30 份檔案。');return;}
  if(!confirm('將建立 V'+(latest.versionNo+1)+'，保留 '+selections.length+' 份群組檔案為合約附件。送出新版後，廠商與已驗證群組成員即可查看。舊版與原始檔不會改變。是否繼續？'))return;
  LINE_ATTACHMENT_REVIEW.importing=true;
  renderLineAttachmentReview();
  try{
    await apiV2('contracts/'+encodeURIComponent(contractId)+'/versions/'+encodeURIComponent(latest.id)+'/line-attachments',{method:'POST',body:{selections}});
    if(WORKFLOW.contract?.id!==contractId)return;
    const detail=await apiV2('contracts/'+encodeURIComponent(contractId));
    if(WORKFLOW.contract?.id!==contractId)return;
    WORKFLOW.detail=detail;WORKFLOW.row.workflowState='draft';WORKFLOW.row.signingStatus='';
    WORKFLOW.row.latestVersion=detail.latestVersion?.versionNo||WORKFLOW.row.latestVersion;
    WORKFLOW.creatingVersion=false;
    LINE_ATTACHMENT_REVIEW={versionId:'',items:[],loading:false,importing:false};
    renderWorkflow();loadLineAttachmentCandidates();
    showPageMessage('已建立 V'+WORKFLOW.row.latestVersion+'；選取檔案已納入合約，尚未送出 LINE。');
  }catch(error){if(WORKFLOW.contract?.id!==contractId)return;LINE_ATTACHMENT_REVIEW.importing=false;renderLineAttachmentReview();alert('保留群組附件失敗：'+error.message);}
}
function existingDocumentBox(kind,label,files,nextVersion,mode){const history=(WORKFLOW.detail?.latestVersion?.documentPackage?.attachments||WORKFLOW.detail?.latestVersion?.snapshot?.documentPackage?.attachments||[]).filter(item=>String(item.category||'')===kind);const items=files.length?files.map(file=>{const id=fileIdentity(file);const href=sourceFileLink(file);const source=file.sourceVersionNo||WORKFLOW.detail?.latestVersion?.versionNo||'';const evidence=file.sourceEvidence||{};const origin=evidence.source==='engineering_phase_1_drawing_library'?'<span class="source-badge">階段一圖庫'+(evidence.drawingVersion?'｜'+esc(evidence.drawingVersion):'')+'</span>':'<span class="file-origin">源自 V'+esc(source)+'</span>';return '<div class="carried-file" data-next-file="'+esc(id)+'">'+(href?'<a target="_blank" rel="noopener" href="'+esc(href)+'">✓ 沿用：'+esc(file.name||file.fileName||label)+'</a>':'<span>✓ 沿用：'+esc(file.name||file.fileName||label)+'</span>')+origin+'<button type="button" class="rowbtn" onclick="toggleNextVersionDocument(\\\''+encodeURIComponent(id)+'\\\',this)">從 V'+nextVersion+' 停用</button></div>';}).join(''):'<div class="file-state">目前沒有有效的'+esc(label)+'</div>';const action=mode==='add'?'新增施工圖':'替換'+label;return '<div class="workflow-box"><h4>'+esc(label)+'</h4><div class="hint">未調整時會直接沿用，不需要重複上傳。</div><div class="carried-files">'+items+'</div><label class="hint">'+esc(action)+'<input type="file" id="wf-file-'+kind+'" onchange="uploadWorkflowFile(\\\''+kind+'\\\')"></label><div class="file-state" id="wf-state-'+kind+'">'+(files.length?'本次尚未選擇新檔，將沿用上列文件':'尚未上傳')+'</div>'+(history.length?'<details class="history-files"><summary>查看 '+history.length+' 份歷史版本證據</summary>'+history.map(item=>'<div>V'+esc(item.sourceVersionNo||'—')+'｜'+esc(item.name||item.fileName||label)+'</div>').join('')+'</details>':'')+'</div>';}

function projectDrawingHref(id){return '/contracts/api/v2/contracts/'+encodeURIComponent(WORKFLOW.contract.id)+'/project-drawings/'+encodeURIComponent(id)+'?tenant='+encodeURIComponent(TENANT)+'&key='+encodeURIComponent(KEY);}
function projectDrawingPickerHtml(){const state=PROJECT_DRAWING_PICKER;const selected=new Set(state.selected||[]);const cards=state.open?(state.items||[]).map(item=>{const checked=item.included||selected.has(item.id);const preview=item.available&&item.mimeType&&item.mimeType.startsWith('image/')?'<img class="project-drawing-thumb" loading="lazy" src="'+esc(projectDrawingHref(item.id))+'" alt="'+esc(item.name)+'">':'<div class="project-drawing-fileicon">'+(item.mimeType==='application/pdf'?'📄':'📐')+'</div>';const file=item.available?'<a target="_blank" rel="noopener" href="'+esc(projectDrawingHref(item.id))+'">'+esc(item.name)+'</a>':'<span>'+esc(item.name)+'</span>';return '<div class="project-drawing-card '+(checked?'selected ':'')+(!item.available?'unavailable':'')+'">'+preview+'<div><label><input type="checkbox" '+(checked?'checked':'')+' '+(item.included||!item.available?'disabled':'')+' onchange="toggleProjectDrawingSelection(\\\''+encodeURIComponent(item.id)+'\\\',this.checked)"> '+esc(item.drawingName||item.name)+'</label><div>'+file+'</div><div class="hint">'+esc(item.version||'未標版本')+'｜'+esc(item.status||'')+'｜'+esc((item.uploadedAt||'').replace('T',' ').slice(0,16))+'</div><div class="hint">'+esc(item.uploader||'未記錄上傳人')+(item.included?'｜已納入目前版本':item.reason?'｜'+esc(item.reason):selected.has(item.id)?'｜已選取，儲存版本時納入':'')+'</div></div></div>';}).join(''):'';return '<div id="project-drawing-picker" class="workflow-box wide"><h4>從工程圖庫選擇施工圖</h4><div class="hint">只會顯示本合約同一工程「階段一：審圖與圖面定版」的版本。選取後沿用原始私有 Drive 檔，不搬移、不刪除；儲存合約版本時會固定檔名、版本、大小與 SHA-256 證據。</div><button type="button" class="rowbtn" onclick="loadProjectDrawingCandidates()" '+(state.loading?'disabled':'')+'>'+(state.loading?'正在讀取…':state.open?'重新整理工程圖庫':'從工程圖庫選擇')+'</button>'+(state.error?'<div class="version-missing">'+esc(state.error)+'</div>':'')+(state.open?'<div class="project-drawing-grid">'+cards+'</div>'+(state.items.length?'':'<div class="file-state">本工程的階段一目前沒有可選圖面。</div>'):'')+'</div>';}
function renderProjectDrawingPicker(){const panel=document.getElementById('project-drawing-picker');if(panel)panel.outerHTML=projectDrawingPickerHtml();}
async function loadProjectDrawingCandidates(){if(!WORKFLOW.contract?.id||PROJECT_DRAWING_PICKER.loading)return;const contractId=WORKFLOW.contract.id;PROJECT_DRAWING_PICKER={...PROJECT_DRAWING_PICKER,contractId,loading:true,open:true,error:''};renderProjectDrawingPicker();try{const items=await apiV2('contracts/'+encodeURIComponent(contractId)+'/project-drawings');if(WORKFLOW.contract?.id!==contractId)return;PROJECT_DRAWING_PICKER={...PROJECT_DRAWING_PICKER,contractId,items,loading:false,open:true,error:''};renderProjectDrawingPicker();}catch(error){if(WORKFLOW.contract?.id!==contractId)return;PROJECT_DRAWING_PICKER={...PROJECT_DRAWING_PICKER,loading:false,open:true,error:'工程圖庫載入失敗：'+error.message};renderProjectDrawingPicker();}}
function toggleProjectDrawingSelection(encodedId,checked){const id=decodeURIComponent(encodedId);const selected=new Set(PROJECT_DRAWING_PICKER.selected||[]);if(checked)selected.add(id);else selected.delete(id);PROJECT_DRAWING_PICKER={...PROJECT_DRAWING_PICKER,selected:[...selected]};renderProjectDrawingPicker();}
...
function renderWorkflow(){
  ...
  if((!latest||WORKFLOW.creatingVersion)&&CAN_MANAGE){
    ...
    if(status==='issued'){html+='<span class="hint">電子簽署：'+esc(workflowStatusLabel(WORKFLOW.row.signingStatus)||'尚未送簽')+'</span>';if(CAN_ISSUE&&['','revoked','expired','declined'].includes(WORKFLOW.row.signingStatus))html+=signerSelect()+'<button class="btn ghost" onclick="workflowIssue(\\'retry-signing\\')">以同一版本重新送簽</button>';if(CAN_ISSUE&&currentPartyAProfileType()==='individual'&&WORKFLOW.row.signingSessionId&&!WORKFLOW.row.partyASignerLineUserId&&!WORKFLOW.row.partyASigned&&['sent','opened','signed'].includes(WORKFLOW.row.signingStatus))html+=partyASignerAssignmentHtml();if(WORKFLOW.row.signingStatus==='confirmed')html+='<span class="hint">我方確認已完成；目前只需產生最終歸檔。</span>';if(CAN_CONFIRM&&['signed','confirmed'].includes(WORKFLOW.row.signingStatus)&&WORKFLOW.row.signingSessionId)html+='<button id="party-a-complete" class="btn" onclick="workflowComplete()">'+(WORKFLOW.row.signingStatus==='confirmed'?'繼續產生最終歸檔':(currentPartyAProfileType()==='individual'?'驗證甲乙雙方線上簽名並產生最終歸檔':'確認簽署並產生最終歸檔'))+'</button>';}
...
function resetProjectDrawingPicker(){PROJECT_DRAWING_PICKER={contractId:WORKFLOW.contract?.id||'',items:[],selected:[],loading:false,open:false,error:''};}
function startNewVersion(){WORKFLOW.files={};WORKFLOW.pendingExclusions=[];WORKFLOW.revisionReview=null;WORKFLOW.creatingVersion=true;resetProjectDrawingPicker();renderWorkflow();setTimeout(updatePaymentSummary,0);}
function startRevisionFromReview(reviewId){const review=(WORKFLOW.reviews||[]).find(item=>item.id===reviewId);if(!review)return alert('找不到這筆草約意見');WORKFLOW.files={};WORKFLOW.pendingExclusions=[];WORKFLOW.revisionReview=review;WORKFLOW.creatingVersion=true;resetProjectDrawingPicker();renderWorkflow();setTimeout(updatePaymentSummary,0);document.getElementById('workflow-body')?.scrollTo({top:0,behavior:'smooth'});}
function cancelNewVersion(){WORKFLOW.files={};WORKFLOW.pendingExclusions=[];WORKFLOW.revisionReview=null;WORKFLOW.creatingVersion=false;resetProjectDrawingPicker();renderWorkflow();}
...
async function createWorkflowDraft(){
  try{const templateVersionId=document.getElementById('wf-template-version')?.value||'';const contractFields=readContractFieldsWithPartyA();const payments=readEditorRows('.payment-row',['label','trigger','percentage','amount','dueDate','dueTime','evidenceRequired']).map((item,index)=>({id:'payment-'+String(index+1).padStart(3,'0'),label:item.label,trigger:item.trigger,percentage:item.percentage?Number(item.percentage):null,amount:item.amount?Number(item.amount):null,dueDate:item.dueDate,dueTime:item.dueTime,evidenceRequired:item.evidenceRequired}));const criteria=readEditorRows('.acceptance-row',['criterion','reference','verificationMethod','passCondition','evidenceRequired']).map((item,index)=>({id:'acceptance-'+String(index+1).padStart(3,'0'),...item}));
    ...
    await apiV2('contracts/'+encodeURIComponent(WORKFLOW.contract.id)+'/versions',{method:'POST',body:{documentPackage:pkg,projectDrawingSelections:PROJECT_DRAWING_PICKER.selected||[],sourceVersionId:WORKFLOW.detail?.latestVersion?.id||'',attachmentExclusions:WORKFLOW.pendingExclusions||[],...(revisionSource?{snapshot:{revisionSource}}:{}),...(templateVersionId?{templateVersionId}:{})}});WORKFLOW.detail=await apiV2('contracts/'+encodeURIComponent(WORKFLOW.contract.id));WORKFLOW.row.workflowState='draft';WORKFLOW.row.latestVersion=WORKFLOW.detail.latestVersion?.versionNo||WORKFLOW.row.latestVersion;WORKFLOW.files={};WORKFLOW.pendingExclusions=[];WORKFLOW.revisionReview=null;WORKFLOW.creatingVersion=false;resetProjectDrawingPicker();renderWorkflow();showPageMessage('新合約版本已保存；工程圖庫施工圖已固定版本與雜湊證據，原始圖庫檔案未搬移或刪除，替換或停用的舊檔仍保留在版本證據中。');
...
function partyAAssetBadges(profile){if(profile.profileType==='individual')return '<span class="asset-badge">每份合約以指定 LINE 帳號簽名</span>';const assets=profile.assets||{};return assets.large_seal?'<span class="asset-badge">✓ 公司大章</span>':'';}
function renderPartyAProfiles(){
  ...
  main.innerHTML='<div class="panel"><h3>甲方主檔</h3><div class="hint">公司甲方保存公司名稱、統編、負責人、地址及公司大章；個人甲方只保存身分資料，不保存可重複使用的簽名。每份合約送簽時都必須另行指定個人甲方的 LINE 簽署帳號，甲方會在同一個受保護簽署頁完成本次合約簽名。</div><div class="workflow-actions"><button class="btn" onclick="showPartyAProfileForm()">＋ 新增甲方</button></div>'+form+'<div class="profile-grid">'+cards+'</div>'+(cards?'':'<div class="empty">尚未建立甲方主檔。</div>')+'</div>';
}
function partyAProfileFormHtml(id){const profile=PARTY_A_PROFILES.find(item=>item.id===id)||{profileType:'company',assets:{}};PARTY_A_UPLOADS=JSON.parse(JSON.stringify(profile.assets||{}));const type=profile.profileType||'company';const seal='<label class="company-only">公司大章（必填）<input type="file" accept="image/png,image/jpeg" onchange="uploadPartyAAsset(&quot;large_seal&quot;,this)"><div id="pa-state-large_seal" class="file-state">'+(PARTY_A_UPLOADS.large_seal?'✓ 已保存 '+esc(PARTY_A_UPLOADS.large_seal.name||'公司大章'):'尚未上傳')+'</div></label>';const individualNote='<div class="individual-only workflow-box"><b>簽名不存主檔</b><div class="hint">此處只保存姓名、身分資料與地址。每份合約送簽時會指定甲方 LINE 帳號，由本人在受保護簽署頁重新簽名。</div></div>';return '<div id="party-a-form" class="workflow-box profile-form" data-type="'+type+'"><h4>'+(id?'編輯甲方主檔':'新增甲方主檔')+'</h4><div class="workflow-grid"><label>甲方類型<select id="pa-type" onchange="partyATypeChanged()"><option value="company" '+(type==='company'?'selected':'')+'>公司（大章）</option><option value="individual" '+(type==='individual'?'selected':'')+'>個人（每份合約簽名）</option></select></label><label>顯示名稱<input id="pa-display" value="'+esc(profile.displayName||'')+'" placeholder="選單中看到的名稱"></label><label>法定名稱／姓名<input id="pa-legal" value="'+esc(profile.legalName||'')+'"></label><label class="company-only">統一編號<input id="pa-tax" value="'+esc(profile.taxId||'')+'" inputmode="numeric" maxlength="8"></label><label class="company-only">負責人<input id="pa-responsible" value="'+esc(profile.responsiblePerson||'')+'"></label><label>代表人／簽約人<input id="pa-representative" value="'+esc(profile.representative||'')+'"></label><label class="individual-only">身分證字號（需要時）<input id="pa-identity" value="'+esc(profile.identityNumber||'')+'"></label><label class="wide">地址<input id="pa-address" value="'+esc(profile.address||'')+'"></label>'+seal+individualNote+'</div><div class="workflow-actions"><button id="pa-save" class="btn" onclick="savePartyAProfile(&quot;'+esc(id)+'&quot;)">儲存甲方</button><button class="btn ghost" onclick="cancelPartyAProfileForm()">取消</button></div></div>';}
...
function workflowSigningStatusFromControl(contract){return ({awaiting_internal_confirmation:'signed',awaiting_archive:'confirmed',archived:'completed',revoked:'revoked',expired:'expired',declined:'declined'})[String(contract?.workflowStatus||'')]||'';}
function applyWorkflowSigningControl(payload){const contract=payload?.contract||payload||{};const status=workflowSigningStatusFromControl(contract);if(status)WORKFLOW.row.signingStatus=status;WORKFLOW.row.signingSessionId=contract.signingSessionId||WORKFLOW.row.signingSessionId;const partyAStatus=String(contract.partyA?.status||'');if(partyAStatus==='signed')WORKFLOW.row.partyASigned=true;else if(['signer_assignment_required','pending_signature','not_issued'].includes(partyAStatus))WORKFLOW.row.partyASigned=false;return Boolean(status||contract.signingSessionId||partyAStatus);}
async function refreshWorkflowSigningState(){try{const control=await apiV2('contracts/'+encodeURIComponent(WORKFLOW.contract.id)+'/control');if(applyWorkflowSigningControl(control)){renderWorkflow();return true;}}catch{}const listed=await apiV2('contracts');const fresh=(listed?.items||[]).find(item=>String(item.id)===String(WORKFLOW.contract?.id));if(!fresh)return false;WORKFLOW.row.signingStatus=fresh.signing_status||'';WORKFLOW.row.signingSessionId=fresh.signing_external_session_id||WORKFLOW.row.signingSessionId;WORKFLOW.row.partyASignerLineUserId=fresh.party_a_signer_line_user_id||'';WORKFLOW.row.partyASigned=fresh.party_a_signed===true;renderWorkflow();return true;}
...
function signerOptions(members,placeholder){return '<option value="">'+placeholder+'</option>'+members.map(m=>'<option value="'+esc(m.userId)+'">'+esc(m.name)+'</option>').join('');}
function signerSelect(){const group=WORKFLOW.project.groups.find(g=>g.id.replace(/-/g,'')===String(WORKFLOW.row.groupId||'').replace(/-/g,''));const members=group?.members||[];let html='<label>乙方 LINE 簽署人<select id="wf-signer">'+signerOptions(members,'指定乙方簽署人')+'</select></label>';if(currentPartyAProfileType()==='individual')html+='<label>甲方 LINE 簽署人<select id="wf-party-a-signer">'+signerOptions(members,'指定個人甲方簽署人')+'</select></label>';return html;}
async function workflowIssue(action){try{const signer=document.getElementById('wf-signer')?.value;if(!signer)throw new Error('請指定目前仍在工程 LINE 群組內的乙方簽署人');const individual=currentPartyAProfileType()==='individual';const partyASigner=individual?(document.getElementById('wf-party-a-signer')?.value||''):'';if(individual&&!partyASigner)throw new Error('個人甲方也必須指定目前仍在工程 LINE 群組內的甲方簽署人');if(partyASigner&&partyASigner===signer)throw new Error('甲方與乙方不可指定同一個 LINE 帳號');const latest=WORKFLOW.detail.latestVersion;const result=await apiV2('contracts/'+encodeURIComponent(WORKFLOW.contract.id)+'/versions/'+encodeURIComponent(latest.id)+'/'+action,{method:'POST',body:{signerLineUserId:signer,...(partyASigner?{partyASignerLineUserId:partyASigner}:{})}});WORKFLOW.row.signingStatus='sent';WORKFLOW.row.signingSessionId=result.sessionId;alert(result.retried?'已使用同一份凍結合約重新送到 LINE 群組':'正式 PDF 已歸檔並送到 LINE 群組；甲乙雙方會依各自指定的 LINE 帳號簽署。');WORKFLOW.detail=await apiV2('contracts/'+encodeURIComponent(WORKFLOW.contract.id));renderWorkflow();}catch(error){alert(error.message);}}
function currentPartyAProfileType(){const latest=WORKFLOW.detail?.latestVersion||{};const pkg=latest.documentPackage||latest.snapshot?.documentPackage||{};return String(pkg.contractFields?.partyAProfileType||pkg.partyAProfileSnapshot?.profileType||'');}
function partyASignerAssignmentHtml(){const group=WORKFLOW.project.groups.find(g=>g.id.replace(/-/g,'')===String(WORKFLOW.row.groupId||'').replace(/-/g,''));const members=group?.members||[];return '<div class="workflow-box"><h4>個人甲方線上簽署</h4><div class="hint">請指定甲方本人使用的 LINE 帳號。綁定後不可換人；甲方會從原群組連結進入自己的簽名區。</div><select id="wf-assign-party-a">'+signerOptions(members,'指定個人甲方簽署人')+'</select><button class="btn ghost" type="button" onclick="workflowAssignPartyASigner()">綁定甲方 LINE 簽署人</button></div>';}
async function workflowAssignPartyASigner(){try{const signer=document.getElementById('wf-assign-party-a')?.value;if(!signer)throw new Error('請選擇個人甲方的 LINE 帳號');await apiV2('signing-sessions/'+encodeURIComponent(WORKFLOW.row.signingSessionId)+'/assign-party-a',{method:'POST',body:{partyASignerLineUserId:signer}});WORKFLOW.row.partyASignerLineUserId=signer;renderWorkflow();alert('已綁定個人甲方 LINE 簽署人。甲方現在可從原工程 LINE 群組連結進入簽名。');}catch(error){try{await refreshWorkflowSigningState();}catch{}alert(error.message);}}
async function workflowComplete(){const individual=currentPartyAProfileType()==='individual';const button=document.getElementById('party-a-complete');try{if(button){button.disabled=true;button.textContent=WORKFLOW.row.signingStatus==='confirmed'?'產生最終歸檔中…':(individual?'驗證甲乙雙方線上簽名並歸檔中…':'確認並歸檔中…');}await apiV2('signing-sessions/'+encodeURIComponent(WORKFLOW.row.signingSessionId)+'/confirm-complete',{method:'POST',body:{}});WORKFLOW.row.signingStatus='completed';await refreshWorkflowSigningState();alert(individual?'甲方與乙方的線上簽名均已驗證；現在可直接開啟最終合約 PDF 與簽署證據收據。':'已完成我方確認；現在可直接開啟最終合約 PDF 與簽署證據收據。');}catch(error){let refreshed=false;try{refreshed=await refreshWorkflowSigningState();}catch{}if(!refreshed&&button){button.disabled=false;button.textContent=individual?'驗證甲乙雙方線上簽名並產生最終歸檔':'確認簽署並產生最終歸檔';}alert(error.message);}}
load();
</script>
</body>
</html>`;
