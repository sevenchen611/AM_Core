...
let PARTY_A_UPLOADS = {};
let PARTY_A_SIGNATURE_PAD = { canvas:null, context:null, drawing:false, pointerId:null, hasInk:false };
...
function partyAAssetBadges(profile){if(profile.profileType==='individual')return '<span class="asset-badge">每份合約現場簽名</span>';const assets=profile.assets||{};return assets.large_seal?'<span class="asset-badge">✓ 公司大章</span>':'';}
...
function partyASigningPanelHtml(){return '<div class="party-a-signing-box"><h4>個人甲方簽名</h4><div class="hint">乙方已完成簽署。請個人甲方現在直接在下方簽名；這份簽名只綁定本合約與本次簽署，不會回存甲方主檔，也不會帶入其他合約。</div><div class="party-a-signature-wrap"><canvas id="party-a-contract-signature" aria-label="個人甲方本次合約簽名區"></canvas></div><label class="party-a-consent"><input id="party-a-signature-consent" type="checkbox"><span>本人確認已閱讀本次合約，並同意以此簽名完成甲方簽署及最終歸檔。</span></label><div class="workflow-actions"><button class="btn ghost" type="button" onclick="clearPartyASignaturePad()">清除重簽</button><button id="party-a-complete" class="btn" type="button" onclick="workflowComplete()">送出甲方簽名並完成雙方歸檔</button></div><div id="party-a-signature-state" class="file-state">尚未簽名</div></div>';}
function initPartyASignaturePad(){const canvas=document.getElementById('party-a-contract-signature');if(!canvas)return;const rect=canvas.getBoundingClientRect();const ratio=Math.max(1,Math.min(2.5,window.devicePixelRatio||1));canvas.width=Math.max(320,Math.round(rect.width*ratio));canvas.height=Math.max(280,Math.round(rect.height*ratio));const context=canvas.getContext('2d');context.setTransform(ratio,0,0,ratio,0,0);context.fillStyle='#fff';context.fillRect(0,0,rect.width,rect.height);context.strokeStyle='#16241b';context.fillStyle='#16241b';context.lineWidth=Math.max(2.5,Math.min(4.5,rect.width/165));context.lineCap='round';context.lineJoin='round';PARTY_A_SIGNATURE_PAD={canvas,context,drawing:false,pointerId:null,hasInk:false,lastX:0,lastY:0};const point=e=>{const box=canvas.getBoundingClientRect();return{x:e.clientX-box.left,y:e.clientY-box.top};};canvas.onpointerdown=e=>{if(e.pointerType==='mouse'&&e.button!==0)return;e.preventDefault();const p=point(e);canvas.setPointerCapture?.(e.pointerId);PARTY_A_SIGNATURE_PAD.drawing=true;PARTY_A_SIGNATURE_PAD.pointerId=e.pointerId;PARTY_A_SIGNATURE_PAD.lastX=p.x;PARTY_A_SIGNATURE_PAD.lastY=p.y;context.beginPath();context.arc(p.x,p.y,context.lineWidth/2,0,Math.PI*2);context.fill();PARTY_A_SIGNATURE_PAD.hasInk=true;document.getElementById('party-a-signature-state').textContent='已簽名；確認內容後可送出並歸檔。';};canvas.onpointermove=e=>{if(!PARTY_A_SIGNATURE_PAD.drawing||PARTY_A_SIGNATURE_PAD.pointerId!==e.pointerId)return;e.preventDefault();const p=point(e);context.beginPath();context.moveTo(PARTY_A_SIGNATURE_PAD.lastX,PARTY_A_SIGNATURE_PAD.lastY);context.lineTo(p.x,p.y);context.stroke();PARTY_A_SIGNATURE_PAD.lastX=p.x;PARTY_A_SIGNATURE_PAD.lastY=p.y;};const end=e=>{if(PARTY_A_SIGNATURE_PAD.pointerId!==e.pointerId)return;e.preventDefault();PARTY_A_SIGNATURE_PAD.drawing=false;PARTY_A_SIGNATURE_PAD.pointerId=null;canvas.releasePointerCapture?.(e.pointerId);};canvas.onpointerup=end;canvas.onpointercancel=end;}
function clearPartyASignaturePad(){initPartyASignaturePad();const state=document.getElementById('party-a-signature-state');if(state)state.textContent='已清除，請重新簽名。';}
async function workflowComplete(){const individual=currentPartyAProfileType()==='individual'&&WORKFLOW.row.signingStatus==='signed';const button=document.getElementById('party-a-complete');try{const body={};if(individual){if(!PARTY_A_SIGNATURE_PAD.hasInk||!PARTY_A_SIGNATURE_PAD.canvas)throw new Error('請先由個人甲方在簽名區完成簽名');if(!document.getElementById('party-a-signature-consent')?.checked)throw new Error('請勾選個人甲方的本次合約確認');body.partyASignatureDataUrl=PARTY_A_SIGNATURE_PAD.canvas.toDataURL('image/png');body.partyASignatureConsent=true;}if(button){button.disabled=true;button.textContent='驗證雙方簽名並歸檔中…';}await apiV2('signing-sessions/'+encodeURIComponent(WORKFLOW.row.signingSessionId)+'/confirm-complete',{method:'POST',body});WORKFLOW.row.signingStatus='completed';alert(individual?'甲方與乙方簽名均已驗證；最終合約 PDF 與證據收據已封存。':'已完成我方確認；最終合約 PDF 與證據收據已封存。');renderWorkflow();}catch(error){if(button){button.disabled=false;button.textContent='送出甲方簽名並完成雙方歸檔';}alert(error.message);}}
load();
</script>
</body>
</html>`;
