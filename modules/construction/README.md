...
## 這個模組做什麼
工程租戶(`tenant.key=engineering`)專屬功能:**回饋單、變更單、工項/空間、工種、預算控制、發包合約、工程儀表板(dashboard 併入本模組)、
...
| 子功能 | 來源檔 / 函式 |
|---|---|
| 回饋單(feedback ticket) | 歷史來源 `src/queue.js`:`createTicket`、`generateTicketNumber`、`listTickets`、`ticketAction`、`linkMessageToTicket`、`appendHistory`;現行端點 `/tickets`、`/tickets/api/*` |
| 變更單(change order) | 歷史來源 `src/queue.js`:`createChangeOrder`、`listChangeOrders`;現行端點 `/tickets/api/change-orders`、`/tickets/api/create-co` |
...
| 工程儀表板 | `dashboard.js`(專案卡/甘特/SOP/照片/未銷項/空間×工種矩陣)＋`master-data.js`(直接建立空間/工種/工項並寫回 Notion) |
...
export default {
  name: 'construction',
  init(platform),
  routes: [ /* /budget/*, /contracts/*, /dashboard/*, /tickets 與 /tickets/api/* */ ],
  classify?(ctx),         // 供 collect 注入的 AI 初判分類器(空間/工項)
  reminderPasses?: [],    // 供 reminders 呼叫的工程到期/擱置 pass
  async tick?(ctx),
};
```
`ctx.tenant` 需帶工程專屬 `dataSources { feedbackTickets, changeOrders, workItems, spaces, budgets, contracts, projects, tasks, meetings }`、`queueAccessKey`、`calendars`。Portal 正式權限鍵為 `am-engineering-budget` / `am-engineering-contract`;舊 `am-buildam-*` 只由租戶設定的過渡 alias 接受。
...
- **設計圖版本庫**:`drawings.js` 將階段①圖面以 append-only 方式串流存入目前租戶的 Google Drive 根目錄下，並用租戶自己的 `designDrawings` Notion 資料源建立可搜尋版本索引；同名圖面分組、最新版置頂，歷史檔不覆寫。
- **合約沿用階段一圖庫**:`contract-project-drawings.js` 讓同租戶、同工程的合約草稿直接選取階段①圖面版本；伺服器解析圖面 ID、重驗私有 Drive、計算 SHA-256，並在合約版本保存來源工程／階段／版本快照，不複製或刪除圖庫原檔。
- **專案 Notion 入口**:專案摘要 API 回傳各 Notion 專案頁自身的 `page.url`，每張工程專案卡片在名稱後方顯示 `Notion ↗`，另開原始頁且不觸發案件卡片切換。
- **甘特圖工項編輯**:甘特圖 API 回傳工項頁 id；點擊左側工項標題或右側時間長條可編輯名稱、預計起訖日與狀態。`/dashboard/api/work-item-edit` 寫回目前租戶的 Notion 工項頁，寫入前驗證案件 scope、工項資料庫歸屬及工項－案件 relation，並留下編輯稽核記錄。
...
歷史上抽自舊工程服務的 `src/server.js`,現已重塑成平台 construction 領域檔(吃 `deps`,不寫死租戶)。
...
- `node tools/dryrun-construction-dashboard-management.mjs`:空間／工種／多空間工項建立與甘特圖工項編輯、日期欄位、批次去重、跨案件阻擋與 API 狀態碼驗證。
