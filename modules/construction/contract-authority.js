...
export async function resolveAuthoritativeSigningGroup(deps, { groupBindingId, projectId, signerLineUserId }) {
  const group = await resolveAuthoritativeContractGroup(deps, { groupBindingId, projectId });
  if (!signerLineUserId) throw authorityError('缺少指定簽署人', 400);
  const signer = Object.entries(group.members).find(([, userId]) => String(userId) === String(signerLineUserId));
...
export async function resolveAuthoritativeContractGroup(deps, { groupBindingId, projectId }) {
  if (!groupBindingId || !projectId) throw authorityError('缺少合約群組或工程專案', 400);
