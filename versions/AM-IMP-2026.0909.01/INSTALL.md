...
3. In the HOZO AM 2.0 group-binding data source, verify exactly one binding is
   canonically named `HOZO 財務群組` and its `成員對照` rich-text field contains
   one exact reviewer-name mapping.
4. Update Rental to send `mentionName` plus the required content-bound
   `retryKey` described in the module README, and to consider a notification delivered
