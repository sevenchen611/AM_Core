...
-- Production reached schema v8 without every v7 constraint replacement.
-- Remove both the original anonymous CREATE TABLE check and any partially
-- installed named replacement before normalizing individual profile assets.
ALTER TABLE engineering_contracts.party_a_profiles
  DROP CONSTRAINT IF EXISTS party_a_profiles_check;
ALTER TABLE engineering_contracts.party_a_profiles
  ...
  FOR item IN
    SELECT conname
     ...
     WHERE conrelid = 'engineering_contracts.party_a_profiles'::regclass
       AND contype = 'c'
       AND (
         pg_get_constraintdef(oid) ILIKE '%signature%'
         OR pg_get_constraintdef(oid) ILIKE '%assets = %'
       )
  LOOP
...
-- Individual master records contain identity data only. Legacy reusable asset
-- references are cleared after the old signature-required check is gone.
...
-- Restore the contract-bound Party A signature artifact value if production
-- skipped the v7 artifact constraint replacement.
...
UPDATE engineering_contracts.schema_meta
   SET version = '2026-09-02.engineering-contract-evidence.v9', installed_at = clock_timestamp()
 WHERE singleton;
