...
function renderClaimFormInventory(basePath, financeBaseUrl) {
  ...
  return CLAIM_FORM_INVENTORY.map((form, index) => {
    ...
    return `<article class="form-card" data-form-key="${form.key}" data-hide-when-unassigned="${form.hideWhenUnassigned ? 'true' : 'false'}"><div class="form-order">${index + 1}</div><div><div class="form-card-head"><h3>${form.name}</h3><span class="pill">${form.status}</span></div><p>${form.description}</p><div class="form-footer"><div class="form-meta"><span>${form.version}</span><code>${form.key}</code><span class="current-version" data-version-for="${form.key}">版本讀取中…</span><span class="assignment-count" data-count-for="${form.key}">讀取適用群組中…</span></div><div class="form-actions">${previewAction}<button type="button" class="manage-groups" data-manage-form="${form.key}">設定適用群組</button></div></div><details class="version-history"><summary>版本歷史</summary><ul data-version-history-for="${form.key}"><li>讀取中…</li></ul></details></div></article>`;
  }).join('');
}
