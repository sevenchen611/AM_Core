...
export function renderClaimsAuthorityAdminPage({ basePath = '/claims-authority', csrfToken = '' } = {}) {
  ...
  document.querySelector('#assigned').onclick=()=>{view='groups';load()};document.querySelector('#unassigned').onclick=()=>{view='unassigned';load()};document.querySelector('#reload').onclick=load;document.querySelector('#syncDiscovery').onclick=syncDiscovery;document.querySelector('#bindSelected').onclick=()=>bindSelected().catch(e=>{error.textContent=e.message});document.querySelector('#close').onclick=()=>document.querySelector('#members').close();load();
  </script></body></html>`;
}
