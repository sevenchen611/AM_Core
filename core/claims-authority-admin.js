...
function renderClaimFormInventory(basePath, financeBaseUrl) {
  ...
  return CLAIM_FORM_INVENTORY.map((form, index) => {
    ...
    return `<article class="form-card" data-form-key="${form.key}" data-hide-when-unassigned="${form.hideWhenUnassigned ? 'true' : 'false'}"><div class="form-order">${index + 1}</div><div><div class="form-card-head"><h3>${form.name}</h3><span class="pill">${form.status}</span></div><p>${form.description}</p><div class="form-footer"><div class="form-meta"><span>${form.version}</span><code>${form.key}</code><span class="current-version" data-version-for="${form.key}">目前 v1</span><span class="assignment-count" data-count-for="${form.key}">讀取適用群組中…</span></div><div class="form-actions">${previewAction}<button type="button" class="manage-groups" data-manage-form="${form.key}">設定適用群組</button></div></div><details class="version-history"><summary>版本歷史</summary><ul data-version-history-for="${form.key}"><li>v1｜已發布唯讀</li></ul></details></div></article>`;
  }).join('');
}
...
export function renderClaimsAuthorityAdminPage({ basePath = '/claims-authority', csrfToken = '', financeBaseUrl = '' } = {}) {
  ...
  function modeSelect(item){const select=document.createElement('select');select.setAttribute('aria-label','設定 '+(item.display_name||'群組')+' 的請款模式');for(const [value,label] of [['external_claim_only','外部廠商－單純請款'],['internal_v3','內部同仁－V3 請款']]){const option=document.createElement('option');option.value=value;option.append(text(label));option.selected=item.claim_mode===value;select.append(option)}select.onchange=async()=>{const next=select.value,previous=item.claim_mode||'internal_v3';if(!confirm('確定立即變更這個群組的請款模式嗎？')){select.value=previous;return}select.disabled=true;try{await mutate('/groups/mode',{groupLookup:item.group_lookup,claimMode:next});item.claim_mode=next}catch(e){select.value=previous;error.textContent=e.message}finally{select.disabled=false}};return select}
  async function load(){error.textContent='';rows.replaceChildren();const target=document.querySelector('#target'),bind=document.querySelector('#bindSelected');target.hidden=view==='groups';bind.hidden=view==='groups';try{if(view==='unassigned'){const targets=await call('/targets');target.replaceChildren();for(const item of targets.items){const option=document.createElement('option');option.value=item.key;option.append(text(item.label));target.append(option)}}const data=await call(view==='groups'?'/groups':'/unassigned');for(const item of data.items){const tr=document.createElement('tr'),pick=document.createElement('td');if(view==='unassigned'){const checkbox=document.createElement('input');checkbox.type='checkbox';checkbox.className='groupPick';checkbox.value=item.group_lookup;checkbox.setAttribute('aria-label','選取 '+(item.display_name||'群組'));pick.append(checkbox)}const mode=document.createElement('td');if(view==='groups')mode.append(modeSelect(item));else mode.append(text('綁定後設定'));tr.append(pick,cell(item.display_name||String(item.group_lookup||'').slice(0,12)),mode,cell(item.state),cell(item.oa_state),cell(item.updated_at||''));const actions=document.createElement('td');if(view==='groups')actions.append(action('成員',()=>members(item.group_lookup)),text(' '),action(item.state==='paused'?'恢復':'暫停',()=>setState(item.group_lookup,item.state==='paused'?'active':'paused'),true));tr.append(actions);rows.append(tr)}}catch(e){error.textContent=e.message}}
  ...
  document.querySelectorAll('[data-manage-form]').forEach(button=>button.onclick=()=>openFormGroups(button.dataset.manageForm));
  async function refreshFormCounts(){for(const form of document.querySelectorAll('[data-form-key]')){try{const data=await call('/forms/'+encodeURIComponent(form.dataset.formKey)+'/groups');const count=data.items.filter(item=>item.assigned).length;form.querySelector('.assignment-count').textContent=count+' 個適用群組';if(count===0&&form.dataset.hideWhenUnassigned==='true'){document.querySelector('#disabledFormList').append(form);document.querySelector('#disabledForms').hidden=false}}catch(e){form.querySelector('.assignment-count').textContent='讀取失敗'}}}
  async function refreshFormHistory(){try{const data=await call('/forms/history');for(const form of data.forms||[]){const current=document.querySelector('[data-version-for="'+CSS.escape(form.key)+'"]'),list=document.querySelector('[data-version-history-for="'+CSS.escape(form.key)+'"]');if(current)current.textContent=form.currentVersionNo?'目前 v'+form.currentVersionNo:'尚未發布';if(list){list.replaceChildren();for(const version of form.versions||[]){const item=document.createElement('li');item.append(text('v'+version.versionNo+'｜'+version.status+'｜'+(version.publishedAt||version.createdAt||'')));list.append(item)}if(!(form.versions||[]).length)list.append(text('尚無版本紀錄'))}}}catch(e){for(const item of document.querySelectorAll('.current-version'))item.textContent='版本讀取失敗'}}
  let managedForm='';async function openFormGroups(formKey){managedForm=formKey;const dialog=document.querySelector('#formGroupsDialog'),list=document.querySelector('#formGroupOptions'),status=document.querySelector('#formGroupStatus');status.textContent='';list.replaceChildren();const data=await call('/forms/'+encodeURIComponent(formKey)+'/groups');for(const item of data.items){const label=document.createElement('label');label.className='group-option';const checkbox=document.createElement('input');checkbox.type='checkbox';checkbox.value=item.group_lookup;checkbox.checked=Boolean(item.assigned);checkbox.disabled=item.state!=='active'||item.oa_state!=='present'||(formKey==='employee_expense'&&item.claim_mode!=='internal_v3');label.append(checkbox,text(item.display_name||String(item.group_lookup).slice(0,12)),text(item.claim_mode==='external_claim_only'?'（外部廠商）':'（內部 V3）'));if(checkbox.disabled)label.append(text('（目前不可發布）'));list.append(label)}dialog.showModal()}
  setTimeout(()=>{document.querySelector('#publishFormGroups').onclick=async()=>{if(!managedForm)return;const status=document.querySelector('#formGroupStatus');if(!confirm('確定要立即正式發布這張請款單的群組設定嗎？'))return;try{const groupLookups=[...document.querySelectorAll('#formGroupOptions input:checked')].map(item=>item.value);const result=await mutate('/forms/'+encodeURIComponent(managedForm)+'/groups/publish',{groupLookups});status.className='status';status.textContent='已正式發布，'+result.assignedCount+' 個群組可使用。';await refreshFormCounts()}catch(e){status.className='error';status.textContent=e.message}};document.querySelector('#closeFormGroups').onclick=()=>document.querySelector('#formGroupsDialog').close();refreshFormCounts();refreshFormHistory()},0);
  </script><dialog id="formGroupsDialog"><h2>設定適用 LINE 群組</h2><p class="publish-note">這裡沒有草稿狀態。按下「正式發布」後立即生效；未設定過的其他群組仍維持目前請款流程。</p><div id="formGroupOptions" class="group-picker"></div><p id="formGroupStatus" role="status"></p><div class="bar"><button id="publishFormGroups" type="button">正式發布</button><button id="closeFormGroups" class="secondary" type="button">關閉</button></div></dialog></body></html>`;
}

export function createClaimsAuthorityAdminHandler({ authority, resolveContext, resolveTarget, listTargets, verifyMutation, renderFormPreview, listFormHistory = async () => ({ forms: [] }), syncDiscovery = async () => ({ ok: true, scanned: 0, verified: 0 }), basePath = '/claims-authority' } = {}) {
  ...
  return async function handle(request) {
    try {
      ...
      if (request.method === 'GET' && url.pathname === `${api}/forms/history`) return json(200, await listFormHistory(context));
      ...
      const formGroupsMatch = url.pathname.match(new RegExp(`^${api}/forms/([A-Za-z0-9_-]+)/groups$`, 'u'));
      if (request.method === 'GET' && formGroupsMatch) return json(200, { items: await authority.listFormGroups({ ...context, formKey: formGroupsMatch[1] }) });
      ...
      if (url.pathname === `${api}/groups/mode`) return json(200, await authority.setGroupClaimMode({ ...context, ...body }));
      const publishFormGroupsMatch = url.pathname.match(new RegExp(`^${api}/forms/([A-Za-z0-9_-]+)/groups/publish$`, 'u'));
      if (publishFormGroupsMatch) return json(200, await authority.publishFormGroups({ ...context, formKey: publishFormGroupsMatch[1], groupLookups: body.groupLookups }));
