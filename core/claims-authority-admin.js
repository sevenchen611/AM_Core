...
function renderClaimFormInventory(basePath, financeBaseUrl) {
  ...
  return CLAIM_FORM_INVENTORY.map((form, index) => {
    ...
    return `<article class="form-card" data-form-key="${form.key}"><div class="form-order">${index + 1}</div><div><div class="form-card-head"><h3>${form.name}</h3><span class="pill">${form.status}</span></div><p>${form.description}</p><div class="form-footer"><div class="form-meta"><span>${form.version}</span><code>${form.key}</code><span class="assignment-count" data-count-for="${form.key}">讀取適用群組中…</span></div><div class="form-actions">${previewAction}<button type="button" class="manage-groups" data-manage-form="${form.key}">設定適用群組</button></div></div></div></article>`;
  }).join('');
}
...
export function renderClaimsAuthorityAdminPage({ basePath = '/claims-authority', csrfToken = '', financeBaseUrl = '' } = {}) {
  ...
  async function refreshFormCounts(){for(const form of document.querySelectorAll('[data-form-key]')){try{const data=await call('/forms/'+encodeURIComponent(form.dataset.formKey)+'/groups');form.querySelector('.assignment-count').textContent=data.items.filter(item=>item.assigned).length+' 個適用群組'}catch(e){form.querySelector('.assignment-count').textContent='讀取失敗'}}}
