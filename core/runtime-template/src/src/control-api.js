...
const HOZO_DATA_SOURCE_PARENT_PAGE_ID = normalizeId(process.env.HOZO_DATA_SOURCE_PARENT_PAGE_ID || '');
...
function buildHozoGovernanceEvent(items) {
  ...
  return {
    ...
    solution: '用單一「公司治理與法規合規收尾」任務追蹤，避免文件項目散落。',
...
function buildHozoSiteProcurementEvent(items) {
  ...
  if (!matched.length) return null;
  return {
    subject: '場域建置與採購可營運條件',
    target: '工程 / 設備 / 備品 / 現場驗收',
    project: '場域建置與資產採購',
    priority: '高',
    impact: '場域與資產沒有定義可營運條件，就無法判斷是否能交付房客或進入營運。',
...
function inferDailyMessageProject(text) {
  if (/公司|合約|銀行|稅務|證照|公證|法規|治理/.test(text)) return '公司治理與法規合規';
  if (/工程|裝修|水電|設備|備品|採購|報價|點收/.test(text)) return '場域建置與資產採購';
  if (/品牌|官網|網域|Email|表單|FAQ|素材|上線/.test(text)) return '品牌官網內容';
  if (/通路|招租|月租|帶看|交接|房務|維運|SOP|房客/.test(text)) return '營運類';
  if (/LINE|Notion|PMS|webhook|自動化|每日報告|附件|OCR|資料治理/.test(text)) return '系統自動化與資料治理';
  return '未分類';
}
