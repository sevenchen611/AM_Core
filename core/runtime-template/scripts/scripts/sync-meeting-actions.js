...
const defaultMeetingsDataSourceId = '';
...
async function syncMeeting(page) {
  ...
  for (const actionItem of extractedItems) {
    const candidate = buildTaskCandidate(actionItem.text, meeting, actionItem);
...
function extractActionItems(text) {
  ...
  for (const rawLine of normalized.split('\n')) {
    const sourceType = isCheckboxActionLine(rawLine) ? 'meeting-checkbox' : 'action-keyword';
    const item = normalizeActionLine(rawLine);
    if (!item || seen.has(item)) {
      continue;
    }
    if (sourceType !== 'meeting-checkbox' && !looksLikeActionItem(item)) {
      continue;
    }
    seen.add(item);
    items.push({
      text: item,
      sourceType,
      requiresExtraConfirmation: sourceType !== 'meeting-checkbox',
    });
  }
...
function isCheckboxActionLine(value) {
  return /^\s*(?:[-*+]\s*)?(?:\[[ xX]\]|□|☐|☑|✅)\s*/u.test(String(value || ''));
}
...
function buildTaskCandidate(itemText, meeting, actionItem = {}) {
  ...
  const fromMeetingCheckbox = actionItem.sourceType === 'meeting-checkbox';
  const summary = [
    `會議：${meeting.name}`,
    `同步識別碼：${syncId}`,
    fromMeetingCheckbox
      ? `判斷：由會議記錄 checkbox 直接列入追蹤，不需額外確認。`
      : `判斷：由會議行動項目轉入候選任務。`,
    riskLevel === 'High' ? '注意：內容可能涉及敏感或高風險事項，需專案負責人確認後再推進。' : '',
  ].filter(Boolean).join('\n');
  ...
  return {
    ...
    properties: compactProperties({
      ...
      確認狀態: selectProperty(fromMeetingCheckbox ? '已確認' : '未確認'),
...
async function maybeCreateProgressReport(meeting, bodyText, actionItems) {
  const actionItemTexts = actionItems.map((item) => typeof item === 'string' ? item : item.text);
  const sourceText = `${meeting.name}\n${meeting.summary}\n${bodyText}\n${actionItemTexts.join('\n')}`;
  ...
  const report = buildProgressReport(meeting, bodyText, actionItemTexts, project);
  if (dryRun) {
