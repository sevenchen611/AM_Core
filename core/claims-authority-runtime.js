...
const NO_FORMS_MESSAGE = '此群組目前尚未發布可用的請款單，請聯絡財務管理員設定。';
...
export function createClaimsAuthorityRuntimeAdapter({ authority, replyLine } = {}) {
  ...
  return {
    async handle({ tenant, binding, event }) {
      ...
      if (result.claim?.reason === 'no_forms_published') {
        await replyLine(event, NO_FORMS_MESSAGE);
        return { ...result, userMessage: 'no_forms_published' };
      }
