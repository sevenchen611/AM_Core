...
const J = (o) => ({ ok: true, status: 200, text: async () => JSON.stringify(o), json: async () => o, headers: { get: () => 'application/json' } });
globalThis.fetch = async (input, opts = {}) => {
  ...
  if (u.host === 'api.line.me' && u.pathname.endsWith('/message/push')) { const b = JSON.parse(opts.body); pushed.push(b.messages?.[0]?.text || ''); return J({}); }
  return J({});
};
...
const { dispatcher } = await bootstrap(env, { tenants, logger: { ...console, log: () => {} } });
const evt = (id, fileName) => ({ type: 'message', message: { id, type: 'file', fileName }, source: { type: 'group', groupId: 'Ceng', userId: 'U1' }, timestamp: 1 });
