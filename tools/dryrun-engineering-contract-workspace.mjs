...
assert.match(pageRes.body, /目前有效文件與付款／驗收資料已從 V/);
...
assert.match(pageRes.body, /舊檔仍保留在版本證據中/);
...
assert.match(pageRes.body, /workflowExpectedStatus/);
assert.match(pageRes.body, /系統已重新同步實際狀態/);
assert.match(pageRes.body, /refreshed\.latestVersion\?\.status===workflowExpectedStatus\(action\)/);
...
assert.match(pageRes.body, /applyWorkflowSigningControl\(await apiV2\('contracts\/'\+encodeURIComponent\(contractId\)\+'\/control'\)\)/);
assert.match(pageRes.body, /awaiting_internal_confirmation:'signed'/);
assert.match(pageRes.body, /awaiting_archive:'confirmed'/);
assert.match(pageRes.body, /archived:'completed'/);
assert.match(pageRes.body, /if\(partyAStatus==='signed'\)WORKFLOW\.row\.partyASigned=true/);
