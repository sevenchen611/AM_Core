...
assert.match(pageRes.body, /合約版本庫/);
assert.match(pageRes.body, /showVersionLibrary/);
assert.match(pageRes.body, /新增合約並建立 V1/);
assert.match(pageRes.body, /startContractVersionFromLibrary/);
assert.match(pageRes.body, /openWorkflow\(created\.id,true\)/);
assert.match(pageRes.body, /＋ 建立 V1/);
assert.match(pageRes.body, /startNewVersion/);
assert.match(pageRes.body, /新版本會先承接目前版本的全部內容/);
