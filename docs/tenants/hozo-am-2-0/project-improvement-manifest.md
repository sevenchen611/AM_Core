...
| AM-IMP-2026.0901.01 | Installed | Direct Finance Claims v3 owner | AM Platform pre-ack persistence, database idempotency, staged retry and LINE notification ledger | Removes the suspended cross-service gateway and operational-memory waiting queue. Keep `Installed` until the Render cutover and finance-group canary pass. |
