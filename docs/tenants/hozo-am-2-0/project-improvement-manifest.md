...
| AM-IMP-2026.0831.05 | Installed | Durable Finance Claims v3 ingress | Tenant-scoped PostgreSQL processing queue, fast leases, cold-start retry and terminal private failure notice | Local persistent-store and gateway retry dry-runs pass. Keep `Installed` until production deploy and a sleeping-downstream LINE canary confirm one v3 card with no legacy fallback or duplicate. |
