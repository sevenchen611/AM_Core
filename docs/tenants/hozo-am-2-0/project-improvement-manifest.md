...
| AM-IMP-2026.0914.01 | Installed | Claims group/form publishing and chooser | Publish each of four claim forms to exact LINE groups and require an applicant-bound chooser before opening the selected form | Tenant-scoped schema, immediate-publish admin UI, durable selector delivery, fixed legacy routing and revalidated V3 entry are implemented and synthetic checks pass. Production deployment and one-group canary remain. |
