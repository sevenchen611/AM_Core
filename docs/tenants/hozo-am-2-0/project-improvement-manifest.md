...
| AM-IMP-2026.0913.01 | Deployed | Claims form management catalog | Read-only inventory of three legacy LIFF modes and the Finance Claims V3 standard form | AM Platform `b3c25b7` is live. Production verification confirmed the 「請款功能管理」 title, all four catalog entries, and successful navigation through 「返回財務後台」 to Rental Finance. |
| AM-IMP-2026.0913.02 | Deployed | Claims form administrator previews | Open the three legacy LIFF modes and V3 standard form from the protected claims management catalog without creating or submitting finance data | Rental PR #265 deployed `70e5bb7` through Cloudflare Actions run `34739213603`; AMCore PR #146 deployed `19c7206` on Render deploy `dep-daj2s6ojo6nc73c1u04g`. Production verification confirmed all four links, a protected legacy preview, and disabled V3 attachment, save, and submit controls. |
| AM-IMP-2026.0914.01 | Deployed | Claims group/form publishing and chooser | Publish each of four claim forms to exact LINE groups and require an applicant-bound chooser before opening the selected form | PR #150 merged as `1b6c7e5`; Render `dep-dajog6gae00c73echq80` is Live. Health returned 200 and the protected admin loaded four assignment counts and six eligible groups. No group was published during deployment verification; the user will select the first canary group. |
| AM-IMP-2026.0914.05 | Deployed | Claim form retirement and immutable version history | Hide the unassigned shared-operating form from the active catalog, preserve published form versions, and bind each claim to its submission-time definition | Rental PRs #276 and #280 deployed through Cloudflare; AM PRs #157, #160 and #161 deployed on Render. Production verification confirmed the second form has zero assigned groups and is retained under disabled history, while every catalog form displays its immutable v1 baseline. |
| AM-IMP-2026.0914.07 | Deployed | Named personal claim templates | Let each verified LINE identity keep up to 20 named reusable templates for each of the three legacy LIFF forms and the V3 standard form | Rental PR #274 deployed `2a7209b` through Cloudflare Actions run `34850265948`; AM Platform PR #156 deployed `8346f57`. Both AM health URLs returned 200, and the protected legacy preview displayed all named-template controls in non-writing preview mode. |
| AM-IMP-2026.0915.02 | Deployed | LINE group member claim auto-onboarding | Allow an observed member of an active published claims group unless individually disabled, and provision a claim-only external Finance identity on first use | Rental PR #281 merged as `b9c9fdf`; Cloudflare Pages run `34989712762` passed. AM PR #168 merged as `41c686a`; Render deploy `dep-dakmd8gae00c73dabqu0` is Live. Production health returned 200 with claims loaded, LINE configured and authorization ready. Bonnie's real LINE login remains the user canary. |
| AM-IMP-2026.0916.01 | Deployed | External and internal claims group modes | External vendor groups use applicant-bound legacy claim forms without Finance V3 onboarding; internal groups retain V3 identity and membership controls | PR #170 merged as `e8383f0`; Render deploy `dep-dal02t15efls73fl0q90` is Live. Health passed. `HOZO vs 葉綠宿` is external claim-only, `HOZO 財務群組` remains internal V3, and the external group is disabled and unselected for the V3 form. A real vendor submission remains the user canary. |
| AM-IMP-2026.0916.02 | Deployed | External claims canonical source and private templates | Send Rental's canonical source and exact opaque group scope for external claims, and allow hashed applicant-owned named templates without V3 membership or administrative privileges | Rental PR #284 deployed `2df8e0f` through Cloudflare run `35063163576`, upgrading production D1 to schema v6 with the external-template table present. AM PR #174 deployed `1143d35` on Render `dep-dal3av0jo6nc73audaog`. The successful claim remained unchanged and the template table was empty before user testing. |
