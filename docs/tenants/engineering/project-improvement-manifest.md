...
| AM-IMP-2026.0830.05 | Installed | LINE external-browser contract review links | External-browser opening for new LINE invitations plus a legacy-link fallback | New invitation URLs use LINE's documented `openExternalBrowser=1` parameter while retaining the opaque token in the fragment. Production deployment and mobile verification are pending. |
