...
async function findTenantProjectByName(tenant, name) {
  ...
  try {
    ...
    logger.warn(`Group onboarding could not resolve project "${name}" for tenant=${tenant.key}: ${error.message}`);
    return '';
  }
}

async function groupBindingSchema(tenant) {
  return platform.notionRequest(`/v1/data_sources/${encodeURIComponent(tenant.dataSources.groupBindings)}`, {
    method: 'GET',
    tenantKey: tenant.key,
  });
}

async function queryGroupBindingRows(tenant, groupId) {
...
async function existingGroupBindingRecords(groupId) {
  const records = [];
  for (const tenant of tenants) {
    if (tenant.runtimeEnabled === false || !tenant.notionConfigured || !tenant.dataSources?.groupBindings) continue;
    const rows = await queryGroupBindingRows(tenant, groupId);
    for (const row of rows) records.push({ tenant, row });
  }
  return records;
}
...
async function maybeHandleGroupOnboardingCommand(event, groupId, resolved = {}) {
  ...
  if (!groupId) {
    await replyLine(event, '請在 LINE 群組內發送綁定指令；一對一聊天室沒有群組 ID，不能建立群組綁定。');
    return true;
  }
  if (command.error) {
  ...
  try {
    ...
    if (foreign.length) {
      throw new Error(`此 LINE 群組 ID 已存在於 ${foreign.map((record) => record.tenant.displayName).join('、')}，為避免跨租戶誤綁已停止。`);
    }
